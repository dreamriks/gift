...
// get the filter states, order states, and pagination variables
	...
	$filter_date = $this->_app->getUserStateFromRequest(LA_COMPONENT.'.filter_date','filter_date',LOG_LAST_28_DAYS,'int');
	$limit = $this->_app->getUserStateFromRequest('global.list.limit', 'limit', $this->_app->getCfg('list_limit'), 'int');
	$limitstart = $this->_app->getUserStateFromRequest(LA_COMPONENT.'.limitstart', 'limitstart', 0, 'int');
	$limitstart = ($limit != 0 ? (floor($limitstart / $limit) * $limit) : 0); // In case limit has been changed
	$filter_order = $this->_app->getUserStateFromRequest(LA_COMPONENT.'.filter_order', 'filter_order', 'datetime');
	$filter_order_Dir = $this->_app->getUserStateFromRequest(LA_COMPONENT.'.filter_order_Dir', 'filter_order_Dir', 'desc');
	$search = $this->_app->getUserStateFromRequest(LA_COMPONENT.'.search','search','','string');
...
function table_exists()
{
	...
	$table = $this->_db->replacePrefix('#__flexicontact_log');
