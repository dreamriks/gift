	...
	function ImportVMart()
	{
		...
		foreach($products as $product)
		{
			...
			if ($existing)
			{
				if ($product->product_name != $existing->title || $product->product_s_desc != $existing->description || $destfile != $existing->image)
				{
					...
					$qry = "UPDATE #__fst_prod SET title = '{$db->getEscaped($product->product_name)}', description = '{$db->getEscaped($product->product_s_desc)}', image = '{$db->getEscaped($destfile)}' WHERE id = '{$existing->id}'";
					$db->setQuery($qry);
					$db->query();
				}
			} else {
				...
				$qry .= $product->$pid . ", ";
				$qry .= "'" . $db->getEscaped($product->product_name) . "', ";
				$qry .= "'" . $db->getEscaped($product->product_s_desc) . "', ";
				$qry .= "'" . $db->getEscaped($destfile) . "', ";
				$qry .= "1, $order, 1, 1, 1)";
				$order++;
			}
		...
		foreach($existingproducts as $product)
		{
			if (!array_key_exists($product->import_id, $pids))
			{
				...
				$qry = "DELETE FROM #__fst_prod WHERE id = {$product->id}";
				$db->setQuery($qry);
				$db->Query();	
			}	
		}
