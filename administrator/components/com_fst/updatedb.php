...
//
	...
	function RelinkMenuItems()
	{
		...
		if ($this->DBIs16())
		{
			...
			$db->setQuery("DELETE FROM #__menu WHERE menutype = 'main' AND link = 'index.php?option=com_fst&view=tests'");
			$db->Query();
		} else {
			$db	= & JFactory::getDBO();
			$db->setQuery("SELECT * FROM #__components WHERE link = 'option=com_fst' AND parent = 0");
			...
			$db->setQuery("DELETE FROM #__components WHERE admin_menu_link = 'option=com_fst&view=tests'");
			$db->Query();
	...
	function SetupPluginDBEntries()
	{
		...
		if (!$this->DBIs16())
		{
			...
			if (!$item)
			{
				$qry = "INSERT INTO #__plugins (name, element, folder, iscore) VALUES ('Search - Freestyle Knoweledge Base','fst_kb','search',0)";
				$db->setQuery($qry);$db->Query();
			...
			if (!$item)
			{
				$qry = "INSERT INTO #__extensions (name, type, element, folder, enabled) VALUES ('Search - Freestyle Knoweledge Base','plugin','fst_cron','search', 0)";
				$db->setQuery($qry);$db->Query();
				$log .= "Adding Knoweledge Base Search plugin<br>";
				$installer->refreshManifestCache($db->insertId());
			}
...
//
		...
		foreach($attachments as &$attach)
		{
			...
			if (array_key_exists($ticketid, $ticketlist))
			{
				...
				if ($best > 0)
				{
					...
					$qry = "UPDATE 	#__fst_ticket_attach SET message_id = {$db->getEscaped($best)} WHERE id = {$db->getEscaped($attachid)}";
					$db->setQuery($qry);
					//echo $qry."<br>";
					...
					$qry = "UPDATE 	#__fst_ticket_attach SET message_id = -1 WHERE id = {$db->getEscaped($attachid)}";
					$db->setQuery($qry);
					//echo $qry."<br>";
	...
	function RegisterModules($path)
	{
		...
		if (count($rows) == 0)
		{
			...
			if (file_exists($filename))
			{
				...
				$name = $xml->name;
				//echo $name."<br>";
				$qry = "INSERT INTO #__extensions (name, type, element, client_id, enabled, access) VALUES ('{$db->getEscaped($name)}', 'module', 'mod_fst_support', 0, 1, 0)";
				$db->setQuery($qry);
				$db->Query($qry);
				//exit;
	...
	function SortAPIKey($username = "", $apikey = "")
	{
		...
		if ($comp)
		{
			...
			$qry = "INSERT INTO #__update_sites (name, type, location, enabled) VALUES ('Freestyle Testimonials Updates', 'collection', 'http://www.freestyle-joomla.com/update/list.php?username={$db->getEscaped($username)}&apikey={$db->getEscaped($apikey)}', 1)";
			$db->setQuery($qry);
			$db->Query();
