...
class FstsViewTemplates extends JView
{
	...
	function display($tpl = null)
	{
		...
		} else if ($what == "save")
		{
			...
			$qry = "REPLACE INTO #__fst_templates (template, tpltype, value) VALUES ('custom', 1, '" . $db->getEscaped($head) . "')";
			$db->setQuery($qry);$db->Query();
			$qry = "REPLACE INTO #__fst_templates (template, tpltype, value) VALUES ('custom', 0, '" . $db->getEscaped($row) . "')";
			$db->setQuery($qry);$db->Query();
			...
			foreach($intpltable as $template)
			{
				...
				$qry = "REPLACE INTO #__fst_templates (template, tpltype, value) VALUES ('{$db->getEscaped($template)}', 2, '{$db->getEscaped($value)}')";
				$db->setQuery($qry);
				$db->Query();
			}
			...
			//exit;
			$link = 'index.php?option=com_fst&view=templates';
			...
			JToolBarHelper::title( JText::_("FREESTYLE_TESTIMONIALS") .' - '. JText::_("SETTINGS") , 'fst_templates' );
