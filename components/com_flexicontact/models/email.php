...
function getBrowser(&$browser_string) 
{
	...
	$pattern = '#(?P<browser>' . implode('|', $browsers).')[/ ]+(?P<version>[0-9]+(?:\.[0-9]+)?)#';
	if (!preg_match_all($pattern, $agent, $matches))
		return 99;
	$i = count($matches['browser'])-1;		// use the last match
	$browser_string = $matches['browser'][$i].' '.$matches['version'][$i];
	$browser_parts = explode(' ',$browser_string);
	return array_search($browser_parts[0], $browsers);
}
