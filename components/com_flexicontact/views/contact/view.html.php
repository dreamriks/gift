...
// from name
	...
	echo '<tr><td class="fc_prompt">'.JText::_('COM_FLEXICONTACT_FROM_NAME').'</td>
		  	<td class="fc_field">
			<input type="text" name="from_name" size="30" value="'.$this->post_data->from_name.'" /> '.
...
// from email address
	...
	echo '<tr><td class="fc_prompt">'.JText::_('COM_FLEXICONTACT_FROM_ADDRESS').'</td>
		  	<td class="fc_field">
			<input type="text" name="from_email" size="30" value="'.$this->post_data->from_email.'" /> '.
...
// subject
	...
	if ($this->config_data->show_subject)
		echo '<tr><td class="fc_prompt">'.JText::_('COM_FLEXICONTACT_SUBJECT').'</td>
			<td class="fc_field">
			<input type="text" name="subject" size="30" value="'.$this->post_data->subject.'" /> '.
...
// the main text area
	...
	if ($this->config_data->area_opt != 'disabled')
		echo "\n".'<tr><td valign="top" class="fc_prompt">'.$this->config_data->area_prompt.'</td>
			<td class="fc_field">
			<textarea name="area_data" rows="'.$this->config_data->area_height.'" cols="'.$this->config_data->area_width.'">'.$this->post_data->area_data.'</textarea>
...
// the agreement required checkbox
	...
	if ($this->config_data->agreement_prompt != '')
		...
		if (($this->config_data->agreement_name != '') and ($this->config_data->agreement_link != ''))
			$link_text = $this->config_data->agreement_prompt.' '.JHTML::link($this->config_data->agreement_link, $this->config_data->agreement_name, 'target="_blank"');
		else
...
// get list of images in images directory
    ...
    $handle = opendir(SITE_IMAGES_PATH);
	if (!$handle)
		{
	...
	while (($filename = readdir($handle)) != false)
    	...
    	if ($filename == '.' or $filename == '..')
    		continue;
    	$imageInfo = @getimagesize(SITE_IMAGES_PATH.DS.$filename);
    	if ($imageInfo === false)
