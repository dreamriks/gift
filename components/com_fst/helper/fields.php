...
{	
	...
	function &GetCustomFields($ticketid,$prod_id,$ticket_dept_id,$maxpermission = 3,$isopen = false)
	{
		...
		$qry .= " (allprods = 1 OR '{$db->getEscaped($prod_id)}' IN (SELECT prod_id FROM #__fst_field_prod WHERE field_id = f.id)) AND ";
		$qry .= " (alldepts = 1 OR '{$db->getEscaped($ticket_dept_id)}' IN (SELECT ticket_dept_id FROM #__fst_field_dept WHERE field_id = f.id)) AND ";
		if ($isopen)
		{
	...
	{
		$db =& JFactory::getDBO();
		$qry = "SELECT * FROM #__fst_field WHERE id = '{$db->getEscaped($fieldid)}'";
		$db->setQuery($qry);
	...
	function get_plugin_from_row(&$row)
	{
		...
		$query = "SELECT value FROM #__fst_field_values WHERE field_id = " . $db->getEscaped($row->id);
		$db->setQuery($query);
		...
		if (count($fields) > 0)
		{
			foreach ($fields as &$field)
			{
				...
				if ($value != "XX--XX--XX")
				{
					...
					{
						$qry = "REPLACE INTO #__fst_ticket_user_field (user_id, field_id, value) VALUES ('{$db->getEscaped($userid)}','";
						$qry .= $db->getEscaped($field['id']) . "','";
						$qry .= $db->getEscaped($value) . "')";
						$db->setQuery($qry);
						$db->Query();
					} else {
						$qry = "REPLACE INTO #__fst_ticket_field (ticket_id, field_id, value) VALUES ('{$db->getEscaped($ticketid)}','";
						$qry .= $db->getEscaped($field['id']) . "','";
						$qry .= $db->getEscaped($value) . "')";
						$db->setQuery($qry);
						$db->Query();
	...
	function StoreField($fieldid, $ticketid, $ticket)
	{
		...
		if (array_key_exists($fieldid, $allfields) && $allfields[$fieldid]['peruser'])
		{
			...
			$qry = "SELECT value FROM #__fst_ticket_user_field WHERE user_id = '{$db->getEscaped($userid)}' AND field_id = '{$db->getEscaped($fieldid)}'";
			$db->setQuery($qry);
			$row = $db->loadObject();
			$qry = "REPLACE INTO #__fst_ticket_user_field (user_id, field_id, value) VALUES ('{$db->getEscaped($userid)}','";
			...
			$qry = "SELECT value FROM #__fst_ticket_field WHERE ticket_id = '{$db->getEscaped($ticketid)}' AND field_id = '{$db->getEscaped($fieldid)}'";
			$db->setQuery($qry);
			$row = $db->loadObject();
			$qry = "REPLACE INTO #__fst_ticket_field (ticket_id, field_id, value) VALUES ('{$db->getEscaped($ticketid)}','";
			$qry .= $db->getEscaped($fieldid). "','";
	...
	function &GetUserValues($userid = 0,$ticketid = 0)
	{
		if (empty($this->user_values))
		{
			...
			if ($userid < 1)
			{
				...
				{
					$qry = "SELECT user_id FROM #__fst_ticket_ticket WHERE id = '{$db->getEscaped($ticketid)}'";
					$db->setQuery($qry);
					$row = $db->loadObject();
			...
			$qry = "SELECT * FROM #__fst_ticket_user_field WHERE user_id ='{$db->getEscaped($userid)}'";
			$db->setQuery($qry);
	...
	function &GetTicketValues($ticketid,$ticket)
	{
		...
		if (!array_key_exists($ticketid,$this->_ticketvalues))
		{
			...
			$db =& JFactory::getDBO();
			$qry = "SELECT * FROM #__fst_ticket_field WHERE ticket_id ='{$db->getEscaped($ticketid)}'";
			$db->setQuery($qry);
	...
	function &Comm_GetCustomFields($ident)
	{
		...
		if ($ident != -1)
		{
			$qry = "SELECT * FROM #__fst_field as f WHERE f.published = 1 AND (f.ident = 999 OR f.ident = '{$db->getEscaped($ident)}') ";
		} else {
