	...
	public function addJS() {
		...
		if ($cart) {
			...
			if ($cart->add($virtuemart_product_ids, $errorMsg )) {
				...
				$this->json->msg .= '&nbsp &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<a class="showcart floatright" href="' . JRoute::_("index.php?option=com_virtuemart&view=cart") . '">' . JText::_('COM_VIRTUEMART_CART_SHOW_MODAL') . '</a>';
				if ($errorMsg) $this->json->msg .= '<div><br>'.$errorMsg.'</div>';
				$this->json->stat = '1';
			} else {
				// $this->json->msg = '<p>' . $cart->getError() . '</p>';
				$this->json->msg = '<a class="continue" href="' . $continue_link . '" >' . JText::_('COM_VIRTUEMART_CONTINUE_SHOPPING') . '</a>';
				$this->json->msg .= '<div><br>'.$errorMsg.'</div>';
				$this->json->stat = '2';
			}
		} else {
