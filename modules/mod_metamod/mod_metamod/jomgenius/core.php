...
class JomGeniusClassCore extends JomGeniusParent {
	...
	function info( $type ) {
		...
		switch( $type ) {
			...
			case 'group'			: return $this->userInfo( $type );
	...
	function pageType() {
		...
		switch ( $this->option ) {
			case 'com_content':
				...
				return '';
			case 'com_user':
				switch ( $this->task ) { // I have seen task being used instead of view. Perhaps older versions of J!1.5?
					case 'lostPassword':
						return 'reset';
					case 'register':
						return 'register';
				...
				switch( $this->view ) {
					...
					case 'login':
						return 'login';
					case 'user':
						return 'user'; // editing personal details
				}
				return '';
			case 'com_contact':
				switch ( $this->view ) {
