...
class JomGeniusClassVirtuemart extends JomGeniusParent {
	...
	function previousPurchaseInfo( $type ) {
		...
		if ( $type == 'previouspurchaseproductids' ) {
			$query = 'SELECT DISTINCT oi.product_id FROM #__vm_order_item oi '
				. ' LEFT JOIN #__vm_orders o ON (oi.order_id = o.order_id AND o.order_status = \'C\' AND oi.order_status = \'C\') '
		...
		if ( $type == 'previouspurchaseproductskus' ) {
			$query = 'SELECT DISTINCT oi.order_item_sku FROM #__vm_order_item oi '
				. ' LEFT JOIN #__vm_orders o ON (oi.order_id = o.order_id AND o.order_status = \'C\' AND oi.order_status = \'C\') '
				. ' WHERE o.user_id = ' . $user->id;
		...
		if ( $type == 'previouspurchaseproductnames' ) {
			$query = 'SELECT DISTINCT oi.order_item_name FROM #__vm_order_item oi '
				. ' LEFT JOIN #__vm_orders o ON (oi.order_id = o.order_id AND o.order_status = \'C\' AND oi.order_status = \'C\') '
				. ' WHERE o.user_id = ' . $user->id;
		...
		if ( substr($type, 0, 33) == 'previouspurchasequantityofproduct' ) {			
			...
			$query = 'SELECT SUM(oi.product_quantity) AS product_quantity FROM #__vm_order_item oi '
				. ' LEFT JOIN #__vm_orders o ON (oi.order_id = o.order_id AND o.order_status = \'C\' AND oi.order_status = \'C\' AND oi.product_id = ' . $product_id . ') '
				. ' WHERE o.user_id = ' . $user->id;
	...
	function pageType() {
		...
		if ( $this->page == "checkout.index") {
			...
			if ( $last_step == "0" ) return "checkout.index#1"; // detect One Page Checkout "cart" page
		}
		if ( $this->page != '' ) {
			return $this->page;
		}
