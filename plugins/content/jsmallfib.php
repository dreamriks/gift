...
class plgContentjsmallfib extends JPlugin
{
	...
	function onPrepareContent(&$article, &$params, $limitstart = null) {
		...
		if (0)
	       	{
			echo "I am user [".$username."] with ID [".$userid."] (cat ID ".$user_catid.") and I have access rights [".$access_rights."]<br /><br />";
		}
		...
		if (strtoupper($repository) == "USERBOUND")
			...
			$repository = "Personal area for user ID ".$userid;
			...
			$repository = "Shared area for group ID ".$user_catid;
		...
		if ($is_path_relative)
		{
			$default_absolute_path = JPATH_ROOT.DS.$this->chosen_decoding(trim($this->_params->def('default_path', 'jsmallfib_top'), "/\\"));
		...
		{
			$default_absolute_path = $this->chosen_decoding(rtrim($this->_params->def('default_path', JPATH_ROOT.DS.'jsmallfib_top'), "/\\"));
		}
		if ($repository)
		{
			...
			if ($is_path_relative && !file_exists($starting_dir))
			{
				if (!($rc = @mkdir ($starting_dir)))
				{
					$text  = "<div id='error'>"
						."<table cellspacing='0' cellpadding='0' border='0' width='100%'>"
						."<tr height='30' valign='middle'>"
						."<td width='60px'><img src=\"".$this->imgdir."warning.png\"></td><td>".JText::sprintf('failed_creating_repository', $this->chosen_encoding($repository), $default_absolute_path)."</td>"
						."</tr>"
						."</table>"
						."</div>";
		...
		$hidden_folders = preg_split("/\s*,+\s*/", $hidden_folders_parameter.", JS_ARCHIVE");
		...
		if($access_rights > 1 && (isset($_GET['override_file']) || isset($_GET['archive_file'])))
		{
			$name = $this->baseName($this->chosen_decoding($_GET['filetoupload']));

			$upload_dir = urldecode($_GET['uploaddir']);
			...
			else if(file_exists($upload_file))    // Check to avoid overwriting existing file /ErikLtz
			{
				if ($allow_file_archiving)
				{
					$error = JText::sprintf('failed_upload_exists_archive',
						$this->baselink."&dir=".urlencode($dir),
						$this->baselink."&dir=".urlencode($dir)."&override_file=1&uploaddir=".$_POST['upload_dir']."&filetoupload=".$_FILES['userfile']['name']."&tmpfiletoupload=".$_FILES['userfile']['tmp_name'],
						$this->baselink."&dir=".urlencode($dir)."&archive_file=1&uploaddir=".$_POST['upload_dir']."&filetoupload=".$_FILES['userfile']['name']."&tmpfiletoupload=".$_FILES['userfile']['tmp_name']);
				}
				else
				{
			...
			while ($it = @readdir($open_dir)) 
			{
				if($it != "." && $it != "..")
				{
					...
					else if(!in_array($it, $hidden_files))
					{
						...
						if (!$matched_prefix && !$matched_extension)
						{
		...
		else
		{
			$text  = "<div id='error'>"
				."<table cellspacing='0' cellpadding='0' border='0' width='100%'>"
				."<tr height='30' valign='middle'>"
				."<td width='60px'><img src=\"".$this->imgdir."warning.png\"></td><td>".JText::sprintf('dir_not_found', $this->chosen_encoding($current_position), $default_absolute_path)."</td>"
				."</tr>"
				."</table>"
				."</div>";
		...
		if($files || $dirs)
		{
			...
			{
				$default_sort_order = $this->_params->def('default_sort_order', "desc");
				if (!strcmp($default_sort_order, "desc"))
				{
					@sort($dirs);
					@usort($files, array($this, "name_cmp_desc"));
				}
		...
		*/
		...
		$text .= "<div id='topinfo'>"
			...
			//."	<td>".JText::_('browsing').": ".$this->chosen_encoding($current_position_links)."</td>"
			."	<td>".JText::_('browsing').": ".$this->chosen_encoding(str_replace("Personal area for user ID ".$userid, "Personal area for ".$username, $current_position_links))."</td>"
			...
			foreach ($dirs as $a_dir)
			{
				...
				else
				{
					$text .= "<tr class='row $row_style' onmouseover='this.className=\"highlighted\"' onmouseout='this.className=\"row $row_style\"'>"
						...
						."	</td>"
						."	<td>"
						."	<a href='".$this->baselink."&dir=".urlencode($dir).DS.urlencode($a_dir)."'>".$this->chosen_encoding($a_dir)."</a>"
						."	</td>"
						."	<td class='size'><img src=\"".$this->imgdir."null.gif\" /></td>"
						."	<td class='changed'><img src=\"".$this->imgdir."null.gif\" /></td>";
					if($access_rights > 1)
					...
					if($access_rights > 3)
					{
						...
						$text .= "<td class='icon'>"
							."<a href=\"javascript:confirmDelfolder('".addslashes($this->baselink)."','".urlencode(addslashes($this->chosen_encoding($dir)))."','".urlencode(addslashes($this->chosen_encoding($a_dir)))."','".addslashes(JText::sprintf('about_to_remove_folder', $this->chosen_encoding(addslashes($a_dir))))."')\">"
			...
			foreach ($files as $a_file)
			{
				...
				else
				{
					...
					else
					{
						...
						// $file_link = "<a href='".$this->baselink."&dir=".urlencode($dir)."&download_file=".urlencode($relative_dir.DS.$a_file["name"])."'>".$this->chosen_encoding($a_file["name"])."</a>";
						$file_link = "<a href='".$this->baselink."&dir=".urlencode($dir)."&download_file=".urlencode($dir.DS.$a_file["name"])."'>".$this->chosen_encoding($a_file["name"])."</a>";
					}
					...
					if($access_rights > 2)
					{
						// we need to utf-8 encode potential special characters to be passed to javascript, because Firefox does not handle this (it works in IE)
						$text .= "<td class='icon'>"
							."<a href=\"javascript:confirmDelfile('".addslashes($this->baselink)."','".urlencode(addslashes($this->chosen_encoding($dir)))."','".urlencode(addslashes($this->chosen_encoding($a_file["name"])))."','".addslashes(JText::sprintf('about_to_remove_file', $this->chosen_encoding(addslashes($a_file["name"]))))."')\">"
		...
		if (is_dir($dir.DS."JS_ARCHIVE"))
		{
			$text .= "<div id='upload'>"
				...
				."<tr height='25'>"
				."	<td>&nbsp;&nbsp;&nbsp;"
				."	<a href='".$this->baselink."&dir=".urlencode($dir).DS."JS_ARCHIVE'>".JText::_('view_archive_folder')."</a>"
				."	</td>";

			if($access_rights > 3)
			{
				// we need to utf-8 encode potential special characters to be passed to javascript, because Firefox does not handle this (it works in IE)
				$text .= "<td class='icon' align='right'>"
					."<a href=\"javascript:confirmDelfolder('".addslashes($this->baselink)."','".urlencode(addslashes($this->chosen_encoding($dir)))."','JS_ARCHIVE','"
		...
		// upload section
		if ($access_rights > 1 && strcmp($this->baseName($dir), "JS_ARCHIVE"))
		{
			$text .= "<div id='upload'>"
		...
		//	."	<td>".JText::_('browsing').": ".$this->chosen_encoding($current_position)."</td>"	// moved browsing to the top [ErikLtz]
			."	<td>&nbsp;</td>"
			.$credits_icon
			."</tr>"
			."</table>"
