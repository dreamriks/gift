...
*
* @package CSVI
* @version $Id: csvi.js 1880 2012-02-10 18:08:44Z RolandD $
 */
...
var Csvi = {
	...
	checkVersion: function() {
		var spinner = jQuery('#checkversion').html("<img src='/administrator/components/com_csvi/assets/images/csvi_ajax-loading.gif' />");
		jQuery.ajax({
			async: false,
			url: 'index.php',
			dataType: 'html',
			data: 'option=com_csvi&task=about.checkversion&format=raw',
			success: function(data) {
				jQuery('#checkversion').html(data);
			},
			error: function(data, status, statusText) {
				jQuery('#checkversion').html(Joomla.JText._('COM_CSVI_ERROR_CHECKING_VERSION'));
				jAlert(statusText+'\r\n'+data.responseText);
			}
		});
	},
