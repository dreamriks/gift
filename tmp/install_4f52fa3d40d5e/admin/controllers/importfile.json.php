 ...
 * @version 	$Id: importfile.json.php 1891 2012-02-11 10:43:52Z RolandD $
 */
...
class CsviControllerImportfile extends JController {
	...
	public function doImport() {
		...
		if (is_object($template)) {
			...
			$overridefile = JPATH_BASE.'/templates/'.$app->getTemplate().'/html/'.$option.'/models/'.$template->get('component', 'options').'/import/'.$template->get('operation', 'options').'.php';
			if (file_exists($overridefile)) $this->addModelPath(JPATH_BASE.'/templates/'.$app->getTemplate().'/html/'.$option.'/models/'.$template->get('component', 'options').'/import');
