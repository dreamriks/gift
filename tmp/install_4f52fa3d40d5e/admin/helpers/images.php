 ...
 * @version 	$Id: images.php 1891 2012-02-11 10:43:52Z RolandD $
 */
	...
	*/
	...
	public function processImage($name, $output_path, $output_name=null) {
		...
		if ($this->_imagedata['isimage']) {
			...
			if ($this->_imagedata['isremote'] && $template->get('save_images_on_server', 'image')) {
				...
				$to = $this->_imagedata['base'].'/'.$this->_imagedata['output_path'].basename($this->_imagedata['name']);
			...
			$csvilog->addDebug(JText::_('COM_CSVI_DEBUG_FILE_IS_NOT_IMAGE').' '.$name);
	...
	public function createThumbnail($original, $output_path, $output_name) {
		...
		if ($template->get('thumb_width', 'image') >= 1 && $template->get('thumb_height', 'image') >= 1) {
			...
			$thumb_folder = $output_path.dirname($output_name);
	...
	private function _cleanupImage() {
		...
		if ($ext == 'jpeg') $ext = 'jpg';
		if ($ext != strtolower($this->_imagedata['extension'])) {
		...
		$csvilog->addDebug('Full name target: '.$this->_imagedata['output_name']);
	}
