 ...
 * @version 	$Id: productexport.php 1891 2012-02-11 10:43:52Z RolandD $
 */
...
class CsviModelProductExport extends CsviModelExportfile {
	...
	public function getStart() {
		...
		// Ingore fields
		$ignore = array('metadesc', 'metakey', 'product_name', 'product_s_desc', 'product_desc', 'slug', 'category_path', 'manufacturer_name',
					...
					'pricewithouttax', 'custom_title', 'custom_value', 'custom_price', 'custom_param');
				...
				while ($record = $csvidb->getRow()) {
					...
					foreach ($export_fields as $column_id => $field) {
						if ($field->process) {
							...
							switch ($fieldname) {
								case 'category_id':
									$fieldvalue = CsviHelper::replaceValue($field->replace, $record->virtuemart_category_id);
								...
								case 'product_url':
									...
									if (is_null($record->product_url) || strlen(trim($record->product_url)) == 0) {
										...
										if ($category_id > 0) {
											...
											$_SERVER['QUERY_STRING'] = 'option=com_virtuemart&Itemid='.$template->get('vm_itemid', 'general', 1, 'int').'&view=productdetails&virtuemart_product_id='.$record->main_product_id.'&virtuemart_category_id='.$category_id;
