 ...
 * @version 	$Id: productimport.php 1896 2012-02-11 14:20:33Z RolandD $
 */
...
class CsviModelProductimport extends CsviModelImportfile {
	...
	public function getStart() {
		...
		if ($this->_tablesexist) {
			...
			foreach ($this->csvi_data as $name => $value) {
				...
				switch ($name) {
					...
					case 'product_price':
						...
						$this->_product_prices->$name = $value;
						break;
					case 'product_weight':
					case 'product_length':
					case 'product_width':
			...
			if (is_null($this->product_currency) && (isset($this->_product_prices->product_price) || isset($this->price_with_tax))) {
		   ...
		   $csvilog->AddStats('skipped', JText::sprintf('COM_CSVI_DATA_EXISTS_PRODUCT_SKU', $this->product_sku));
			...
			// User wants to add or update the product
				...
				if ($this->_productQuery()) {
					...
					if (isset($this->_product_prices->product_price) || isset($this->price_with_tax)) $this->_priceQuery();
					...
					if (isset($this->custom_title)) $this->_processCustomFields();
					...
					if (isset($this->category_path) || isset($this->category_id)) {
						if (($this->category_ids && !$this->child_product) || $this->category_id) {
							...
							$csvilog->addDebug(JText::_('COM_CSVI_DEBUG_CHILD_PRODUCT_NO_HANDLING'));
						}
					}
				}
			}
	 ...
	 * @since 		3.0
	 */
	private function _productDiscount($value) {
		// Determine if the discount field is a percentage
		if ($value) {
			if (substr($value,-1,1) == "%") $this->_calcs->calc_value_mathop = '+%';
		}

		// Set the discount amount
		if ($this->_calcs->calc_value_mathop) {
			$this->_calcs->calc_value = substr($this->toPeriod($value), 0, -1);
		}
		else $this->_calcs->calc_value = $this->cleanPrice($value);
	}
	...
	private function _productPackaging() {
		$this->_products->product_packaging = (($this->product_box<<16) | ($this->product_packaging & 0xFFFF));
	}
	...
	private function _productQuery() {
		...
		// Store the debug message
		$csvilog->addDebug(JText::_('COM_CSVI_PRODUCT_LANG_QUERY'), true);
	...
	private function _manufacturerImport() {
		...
		$jinput = JFactory::getApplication()->input;
		$csvilog = $jinput->get('csvilog', null, null);
		$csvilog->addDebug(JText::_('COM_CSVI_DEBUG_MANUFACTURER_IMPORT'));
		...
		if ($this->_manufacturers_lang->check()) {
			...
			$this->_manufacturers_lang->store();

			// Set the manufacturer ID
			$this->_manufacturers->virtuemart_manufacturer_id = $this->_manufacturers_lang->virtuemart_manufacturer_id;

			// Set the manufacturer category
			$this->_manufacturers->virtuemart_manufacturercategories_id = 1;

			// Check if a manufacturer exists
			$this->_manufacturers->check();
	...
	private function _priceQuery() {
		...
		if ($this->child_product && $this->_product_prices->product_price == 0) {
			$csvilog->addDebug(JText::_('COM_CSVI_DEBUG_CHILD_NO_PRICE'));
		}
		else {
			// Check if the price is including or excluding tax
			if ($this->product_tax && $this->price_with_tax && is_null($this->_product_prices->product_price)) {
				if (strlen($this->price_with_tax) == 0) $this->_product_prices->product_price = null;
				else $this->_product_prices->product_price = $this->price_with_tax / (1+($this->product_tax/100));
			}
			else if (strlen($this->_product_prices->product_price) == 0) $this->product_price = null;
			...
			if (is_null($this->_product_prices->product_price)) {
				// Delete the price
				...
				if (!$this->_product_prices->check()) {
					...
					if (!$this->_product_prices->get('product_override_price')) $this->_product_prices->product_override_price = 0;
	...
	private function _processCalc() {
		...
		if (!is_null($this->_calcs->calc_value) && $this->_calcs->calc_value > 0) {
			...
			if (!$this->_calcs->store()) {
				$csvilog->addDebug(JText::_('COM_CSVI_DEBUG_ADD_DISCOUNT'), true);
				return false;
			}
			// Fill the product information with the discount ID
			$this->_product_prices->product_discount_id = $this->_calcs->virtuemart_calc_id;
		}
		else if ($this->_calcs->calc_value == 0) {
			$this->_product_prices->product_discount_id = 0;
		}
	...
	*/
	...
	private function _processCustomFields() {
		...
		foreach ($values as $key => $value) {
			...
			if (isset($params[$key])) $this->_product_customfields->custom_param = $params[$key];
			
			// Check for an existing entry
			if (!$this->_product_customfields->check()) {
				$this->_product_customfields->created_on = $this->date->toMySQL();
				$this->_product_customfields->created_by = $this->user->id;
			}
			
			// Set a modified date
			if (!isset($this->modified_on)) {
				$this->_product_customfields->modified_on = $this->date->toMySQL();
